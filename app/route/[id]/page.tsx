...
export default function RoutePage() {
  ...
  const drawRoute = async (mapInstance: any, coordinates: number[][]) => {
    ...
    try {
      const transport = route.pointsConnect || "walking"
  ...
  `
            ...
            if (landmarksWithCoords.length > 0) {
              ...
              // Draw route connecting all landmarks for route type
  ...
  function mapLabelToType(label: string): string {
    ...
    if (l.includes("landmark") || l.includes("monument") || l.includes("gate") || l.includes("parliament"))
      return "monuments"
  ...
  const getWhatYouWillSee = (routeId: string) => {
    ...
    const data = whatYouWillSeeData[routeId] || whatYouWillSeeData["0"]
  ...
  const copyRouteLink = () => {
    const routeUrl = `${window.location.origin}/route/${routeId}`
  ...
  const shareToSocial = (platform: string) => {
    const routeUrl = `${window.location.origin}/route/${routeId}`
    const text = `Check out this amazing route: ${route?.name} in ${route?.location}`
                  ...
                  <div className="mb-6">
                    <div className="flex items-center justify-between py-2 px-2 bg-[#121214] border border-[#2c2c30] rounded-full pl-4">
                      <span className="text-gray-300 text-sm truncate">
                        {`${window.location.origin}/route/${routeId}`}
                      </span>
                      <button
